...
train_data_dir = 'q3_data/train'  # Directory for training data
image_paths_train = []
labels_train = []
...
train_dataset = CatDogDataset(image_paths_train,
                              labels_train,
                              transform=transform)
train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)

# 4. Load the testing dataset
test_data_dir = 'q3_data/test'  # Directory for testing data
image_paths_test = []
labels_test = []
...
test_dataset = CatDogDataset(image_paths_test,
                             labels_test,
                             transform=transform)
...
for epoch in range(num_epochs):
    ...
    for images, labels in train_loader:
        labels = labels.float().view(-1, 1).to(
            device)  # Reshape labels for BCELoss
    ...
    if True:
        ...
        print(f'Test accuracy of the model every epoch: {accuracy:.2f}%')
